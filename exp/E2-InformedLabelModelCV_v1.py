...
# Cross validation
results = np.empty((len(all_params), n_folds, 4), dtype=float)
results[:] = np.nan
...
for fold, (train_dev_idx, test_idx) in enumerate(kf.split(L_data_local)):
    ...
    for params in all_params:
        ...
        if prev_sig != params[0]:
            deps = CDGAM(L_dev, k=2, sig=params[0], policy="old", verbose=False, return_more_info=False)
            prev_sig = params[0]
